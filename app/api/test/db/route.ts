...
export async function GET(request: NextRequest) {
  try {
    ...
    let usersTest: { success: boolean; error: string | null; count: number } = { success: false, error: null, count: 0 }
    try {
    ...
    let documentsTest: { success: boolean; error: string | null; count: number } = { success: false, error: null, count: 0 }
    try {
      const { data, error, count } = await supabase
        .from('documents')
    ...
    let categoriesTest: { success: boolean; error: string | null; count: number } = { success: false, error: null, count: 0 }
    try {
      const { data, error, count } = await supabase
        .from('categories')
        .select('id', { count: 'exact' })
    ...
    let versionsTest: { success: boolean; error: string | null; count: number } = { success: false, error: null, count: 0 }
    try {
      const { data, error, count } = await supabase
    ...
    let logsTest: { success: boolean; error: string | null; count: number } = { success: false, error: null, count: 0 }
    try {
      const { data, error, count } = await supabase
        .from('system_logs')
        .select('id', { count: 'exact' })
