...
export async function POST(
  ...
  try {
    ...
    if (versionError || !version) {
      ...
      await logAction(user.id, 'Document Version Create', `${params.id}`, 'failed', getClientIp(request))
      return NextResponse.json(
    ...
    await logAction(user.id, 'Document Version Create', `${document.name} v${nextVersion} (${params.id})`, 'success', getClientIp(request))
